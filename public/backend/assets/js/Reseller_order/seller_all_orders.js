$(document).ready(function () {
    var logo = $("#logo").val();
    var table = $("#tblAllSellerOrder").DataTable({
        processing: true,
        serverSide: true,
        ordering: false,
        ajax: {
            url: "getallsellerordersdata",
            data: function(d) {
                d.datefilter = $('#datefilter').val(); // Pass date filter value
            },
        },
        ...
        columns: [
            ...
            },
            {
                data: "order_number",
                render: function (data, type, row) {
                    let html = '<a class="text-dark" href="' + baseUrl + '/seller/order/details/' + row.id + '">' + row.order_number + '</a>';
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    let html =
                        ...
                        row.userName +
                        ' <div class="text-muted bg-light p-1"><small>' +
                        (row.userphone != null ? row.userphone + " " : "") +
                        "</small>";

                    if (row.usercity || row.userstate) {
                        html +=
                            " / <small>" +
                            (row.usercity != null ? row.city_name + ", " : "") +
                            (row.userstate != null ? row.state_name : "") +
                            "</small>";
                    }
            ...
            { data: 'userphone', visible: false, searchable: true },
            {
                data: "name",
                render: function (data, type, row) {
                    let html =
            ...
            { data: "shipping_charge" },
            {
                data: "margin_amount",
                render: function (data, type, row) {
                    var amount = row.margin_amount ? row.margin_amount : "0";
                    return amount;
                },
            },
            { data: "amount" },
            ...
            { 
                ...
                render: function (data, type, row) {
                    var orderId = data.id; 
                    var html = '<a href="' + baseUrl + '/order/print/modal/' + orderId + '" class="btn btn-success" target="_blank">Print</a>';
    ...
    function updateRoute() {
        ...
        if (selectedDate) {
            dateexportURL = exportURL + "?date=" + selectedDate;
        } else {
            dateexportURL = exportURL; 
        }

        $('#export-excel').attr('href', dateexportURL);
    }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        Swal.fire({
                            ...
                            $("#tblAllSellerOrder").DataTable().ajax.reload();
                        });
                    },
