(function ($) {
    ...
    if (productCount >= 6) {
        $(window).scroll(function () {
            ...
            // }
            if (!stop) {
                if($(".shop-area").offset() != undefined) {
                    var hT = $(".shop-area").offset().top,
                        hH = $(".shop-area").outerHeight(),
                        wH = $(window).height(),
                        wS = $(this).scrollTop();
                    if (wS > hT + hH - wH) {
                        page++;
...
/*-------view-passwor------->*/
    ...
    var activeId = $("#instafeed"),
        ...
        myTemplate =
            '<div class="single-instafeed"><a href="{{link}}" target="_blank" id="{{id}}"><img src="{{image}}" /></a></div>';
    if (activeId.length) {
        var accessTokenID =
                "IGQVJWLXU1Ri1JbjE0RlhlRmVZAMy1mRllxUzJWZAG5najYxWUxLcGl1SV80UHNiN2ZAaUFdHTVllMEh6YjRucTZAIY09TQlowdGlScG9taHhWNHJwaXQzOVZAwUDdsZAHdqTjhTcHB5ZA2I5QWRVZAVZAtYjZA2SgZDZD",
    ...
    // if(readCookie('buyQty') != undefined) {
    //     alert(readCookie('buyQty'));
    // }
    ...
    // $(".payment-box.online").css("display", "block");
    // $("#state_name").find('select').trigger('change');
    ...
    //Remove easyzoom option in mobile view
