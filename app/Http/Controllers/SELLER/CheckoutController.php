  ...
  public function CheckoutStore(Request $request)
  {
    ...
    try {
      ...
      if ($request->payment_method == 'razorpay') {
        ...
        DB::commit();
        return view('seller.payment.razorpay_button', compact('response', 'data', 'carts', 'cartTotal', 'coupon_discount', 'coupon_id', 'coupon_amount'));
      } else {
        ...
        $data['shipping_email'] = $request->shipping_email;
        $data['shipping_phone'] = $request->shipping_phone;
        $data['collectcash_amount'] = $request->collectcash_amount;
        $data['margin_amount'] = $request->hdmargin_amount;
        $data['alternative_number'] = $request->alternative_number;
        $data['door_no'] = $request->door_no;
        $data['street_address'] = $request->street_address;
        $data['city_name'] = $request->city_name;
        ...
        $carts = Cart::where('user_id', Auth::user()->id)->get();
        $cartTotal = Cart::where('user_id', Auth::user()->id)->sum('total');
        $margin_amount = $request->hdmargin_amount;
        if ($request->cart_true == 1) {
          $totQty = Cart::where('user_id', Auth::user()->id)->sum('qty');
        } else {
          $totQty = $request->buy_now_product_qty;
        ...
        $shipping_charge = ($totQty * $shipping_charge);
        $cart_total = $cartTotal + $shipping_charge + $margin_amount;
        ...
        DB::commit();
        return view('seller.payment.cash', compact(
          'data',
          'cartTotal',
          'carts',
          'cart_true',
